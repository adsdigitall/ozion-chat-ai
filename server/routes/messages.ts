...
router.post('/send', async (req: Request, res: Response) => {
  try {
    const { conversationId, type, text, templateName, templateLanguage = 'pt_BR', templateParams } = req.body;
    if (!conversationId || !type) {
      return res.status(400).json({ error: 'conversationId and type are required' });
    }
    const sb = getSupabase();
    const { data: conversation, error: convErr } = await sb.from('conversations').select('*').eq('id', conversationId).single();
    ...
    const { error: msgErr } = await sb.from('messages').insert({
      ...
      direction: 'outbound',
      type: type || 'text',
      content: type === 'text' ? text : JSON.stringify({ template: templateName }),
      status: 'sent',
