...
type Role = "admin_master" | "client" | "manager" | "attendant";
...
const roles: Record<Role, { icon: LucideIcon; label: string; color: string }> = {
  admin_master: { icon: Crown, label: "Admin Master", color: "text-amber-400 bg-amber-500/10" },
  client: { icon: Shield, label: "Cliente", color: "text-emerald-400 bg-emerald-500/10" },
  manager: { icon: UserCog, label: "Gestor", color: "text-blue-400 bg-blue-500/10" },
  attendant: { icon: User, label: "Atendente", color: "text-zinc-400 bg-zinc-500/10" },
};
...
export default function UsersPage() {
  ...
  const [form, setForm] = useState<{ name: string; email: string; role: Role }>({ name: "", email: "", role: "attendant" });
  ...
  const invite = async (event: FormEvent) => {
    ...
    try {
      ...
      setModal(false); setForm({ name: "", email: "", role: "attendant" });
    } catch (inviteError) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{(["admin_master", "client", "manager", "attendant"] as Role[]).map((item) => { const config = roles[item]; return <div key={item} className="oz-card rounded-xl p-4"><config.icon className="w-5 h-5 text-zinc-400 mb-3" /><p className="text-2xl font-bold text-white">{users.filter((user) => user.role === item).length}</p><p className="text-xs text-zinc-500">{config.label}</p></div>; })}</div>
